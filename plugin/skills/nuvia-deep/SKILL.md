...
Para mapear a rede societária de alguém (todas as empresas em que consta como sócio): use `nome_socio` (não `socio`!), resolvendo o nome em `brazil_socio` antes. Detalhes em `../nuvia/references/mapeamentos-br.md`.
...
Sempre inclua os links (ver skill `nuvia`): o **site** e o **LinkedIn da empresa** no cabeçalho da empresa, e o **LinkedIn de cada contato** na linha dele — preferindo o handle legível do `linkedin_url_array` (não a URN `ACoAA…`).
